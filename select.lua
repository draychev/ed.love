...
function Text.clip_selection(State, line_index, apos, bpos)
    ...
    if Text.lt1({line = maxl, pos = maxp}, {line = minl, pos = minp}) then
        minl, maxl = maxl, minl
        minp, maxp = maxp, minp
    end
    ...
    local a_ge = Text.le1({line = minl, pos = minp},
                          {line = line_index, pos = apos})
    local b_lt = Text.lt1({line = line_index, pos = bpos},
                          {line = maxl, pos = maxp})
    ...
    elseif b_lt then
        assert(minl == line_index,
               ('minl %d not equal to line_index %d'):format(minl, line_index))
        return minp, bpos
    else
        assert(minl == maxl and minl == line_index,
               ('minl %d, maxl %d and line_index %d are not all equal'):format(
                   minl, maxl, line_index))
        return minp, maxp
    end
end
...
function Text.draw_highlight(State, line, x, y, pos, lo, hi)
    if lo then
        ...
        love.graphics.rectangle('fill', x + lo_px, y, State.font:getWidth(s),
                                State.line_height)
...
function Text.mouse_pos(State)
    ...
    return screen_bottom1.line,
           Text.pos_at_end_of_screen_line(State, screen_bottom1)
end
...
function Text.delete_selection_and_record_undo_event(State)
    ...
    record_undo_event(State, {
        before = before,
        after = snapshot(State, State.cursor1.line)
    })
end
...
function Text.delete_selection_without_undo(State)
    ...
    elseif minl == maxl then
        if minp > maxp then minp, maxp = maxp, minp end
    end
    ...
    if Text.lt1(State.cursor1, State.screen_top1) then
        ...
        State.screen_top1.pos = Text.pos_at_start_of_screen_line(State,
                                                                 State.cursor1)
    end
    State.selection1 = {}
    ...
    if minl == maxl then
        -- ?     print('minl == maxl')
        State.lines[minl].data =
            State.lines[minl].data:sub(1, min_offset - 1) ..
                State.lines[minl].data:sub(max_offset)
        return
    end
    ...
    State.lines[minl].data = State.lines[minl].data:sub(1, min_offset - 1) ..
                                 rhs
end
...
function Text.selection(State)
    ...
    for i = minl + 1, maxl - 1 do table.insert(result, State.lines[i].data) end
