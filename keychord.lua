...
function App.combine_modifiers(key)
    if love.keyboard.isModifierActive then -- waiting for LÖVE v12
        if key:match('^kp') then key = App.translate_numlock(key) end
    end
    local result = ''
    if App.ctrl_down() then result = result .. 'C-' end
    if App.alt_down() then result = result .. 'M-' end
    if App.shift_down() then
        result = result .. 'S-' -- don't try to use this with letters/digits
    end
    if App.cmd_down() then result = result .. 's-' end
    result = result .. key
    return result
end

function App.any_modifier_down()
    return App.ctrl_down() or App.alt_down() or App.shift_down() or
               App.cmd_down()
end

function App.ctrl_down() return App.key_down('lctrl') or App.key_down('rctrl') end

function App.alt_down() return App.key_down('lalt') or App.key_down('ralt') end

function App.shift_down() return
    App.key_down('lshift') or App.key_down('rshift') end

function App.cmd_down() return App.key_down('lgui') or App.key_down('rgui') end
...
Numlock_on = {kpenter = 'enter', kpdel = 'delete'}
...
function array.find(arr, elem)
    if type(elem) == 'function' then
        for i, x in ipairs(arr) do if elem(x) then return i end end
    else
        for i, x in ipairs(arr) do if x == elem then return i end end
    end
    return nil
end
