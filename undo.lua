...
function record_undo_event(State, data)
    ...
    for i = State.next_history, #State.history do State.history[i] = nil end
end
...
function snapshot(State, s, e)
    ...
    for i = s, e do table.insert(event.lines, deepcopy(State.lines[i])) end
    return event
end
...
function patch(lines, from, to)
    ...
    for i = from.end_line, from.start_line, -1 do table.remove(lines, i) end
...
function deepcopy(obj, seen)
    ...
    seen[obj] = result
    for k, v in pairs(obj) do result[deepcopy(k, seen)] = deepcopy(v, seen) end
    return result
end
