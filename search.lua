...
function Text.draw_search_bar(State)
    ...
    love.graphics.rectangle('fill', 20, y - 6, App.screen.width - 40, h + 2, 2,
                            2)
    love.graphics.setColor(0.6, 0.6, 0.6)
    love.graphics.rectangle('line', 20, y - 6, App.screen.width - 40, h + 2, 2,
                            2)
...
function Text.search_next(State)
    ...
    local offset = find(curr_line, State.search_term, curr_offset, --[[literal]]
                        true)
    if offset then State.cursor1.pos = utf8.len(curr_line, 1, offset) end
    if offset == nil then
        -- search lines below cursor
        for i = State.cursor1.line + 1, #State.lines do
            local curr_line = State.lines[i].data
            offset =
                find(curr_line, State.search_term, --[[from start]] nil, --[[literal]]
                     true)
            if offset then
    ...
    if offset == nil then
        ...
        if pos and pos < State.cursor1.pos then State.cursor1.pos = pos end
    end
    if offset == nil then
    ...
    if Text.lt1(State.cursor1, State.screen_top1) or
        Text.lt1(screen_bottom1, State.cursor1) then
...
function Text.search_previous(State)
    ...
    local curr_offset = Text.offset(curr_line, curr_pos)
    local offset =
        rfind(curr_line, State.search_term, curr_offset - 1, --[[literal]] true)
    if offset then State.cursor1.pos = utf8.len(curr_line, 1, offset) end
    if offset == nil then
        ...
        for i = State.cursor1.line - 1, 1, -1 do
            local curr_line = State.lines[i].data
            offset =
                rfind(curr_line, State.search_term, --[[from end]] nil, --[[literal]]
                      true)
            if offset then
                State.cursor1 = {line = i, pos = utf8.len(curr_line, 1, offset)}
                break
...
function rfind(s, pat, i, plain)
    ...
    assert(endpos >= #pat,
           ('rfind: endpos %d should be >= #pat %d at this point'):format(
               endpos, #pat))
    return endpos - #pat + 1
end
